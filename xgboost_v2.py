...
OUT_DIR     = r"D:\video_project\xgb_out_new_v2"

CONF_MIN   = 0.80
SEED       = 42

TOP_AUS = ["AU07_r","AU04_r","AU14_r","AU45_r","AU06_r","AU10_r","AU12_r","AU01_r","AU17_r","AU26_r","AU25_r","AU02_r"]
FEATURE_MODE = "all"  # "all" | "aus_all" | "casme_top"

# Face handling: "dominant" (recommended) or "mean_all_faces"
FACE_MODE = "dominant"

# Calibration: set True if you care about probabilities across domains
...
def infer_subject_id(video_id: str) -> str | None:
    """
    Best-effort subject extraction. Adjust if your naming differs.
    Tries patterns like: S01_..., subject01..., p12..., person3...
    """
    s = str(video_id)
    m = re.search(r"(?:^|[_\-])(s\d{1,3}|subject\d{1,3}|p\d{1,3}|person\d{1,3})(?:[_\-]|$)", s, flags=re.I)
    if m:
        return m.group(1).lower()
    # fallback: sometimes first token is subject-like
...
def aggregate_face_track(g: pd.DataFrame, feat_cols: list[str]) -> dict:
    # sort frames to make diffs meaningful
    ...
    # metadata for selecting dominant face
...
def aggregate_per_video(df: pd.DataFrame, feat_cols: list[str], face_mode: str = "dominant") -> pd.DataFrame:
    ...
    for vid, clip in df.groupby("video_id", sort=False):
        ...
        for _, g in clip.groupby("face_id", sort=False):
            d = aggregate_face_track(g, feat_cols)
            face_rows.append(d)
        ...
        if face_mode == "dominant":
            # pick face with most frames; tie-break by confidence
...
def build_video_table(csv_path: str, mode: str, face_mode: str) -> tuple[pd.DataFrame, list[str]]:
    ...
    # optional: subject_id for group split
    ...
    # carry subject_id at video level (mode of frames)
...
def split_train_valid_test(df: pd.DataFrame, seed: int = 42):
    """
    Prefer group split by subject_id when available (prevents leakage).
    Otherwise stratified random split by label.
    """
    rng = np.random.RandomState(seed)

    if "subject_id" in df.columns and df["subject_id"].notna().any():
        # group split: 60/20/20 using two-stage GroupShuffleSplit
        ...
        valid_df = rest.iloc[va_idx].reset_index(drop=True)
        test_df  = rest.iloc[te_idx].reset_index(drop=True)
        return train_df, valid_df, test_df

    # fallback: your original stratified per-class split
...
def find_best_threshold_f1(y_true: np.ndarray, proba: np.ndarray):
    # scan thresholds and maximize F1 (actually does what the name says)
...
def align_and_impute(train_df: pd.DataFrame, other_df: pd.DataFrame, feat_cols: list[str]):
    """
    Reindex columns and impute missing using TRAIN means (not zeros).
    """
    train_means = train_df[feat_cols].mean(axis=0)

    other = other_df.set_index(["video_id", "label"])
    other = other.reindex(columns=feat_cols)
    # fill missing columns/values with train means
...
def main():
    ...
    print("FEATURE_MODE =", FEATURE_MODE, "| FACE_MODE =", FACE_MODE, "| CALIB =", USE_CALIBRATION)
    ...
    # quick sanity: labels
    print("DOLOS labels:", sorted(dolos["label"].unique().tolist()))
    ...
    # Optional calibration using VALID set only (no leakage into test)
    if USE_CALIBRATION:
        ...
        calib.fit(Xva, yva)
        model_for_pred = calib
        print("Calibration: isotonic fitted on DOLOS valid (FrozenEstimator).")
    else:
        model_for_pred = model

    # threshold tuning on DOLOS VALID ONLY (maximize F1)
    p_val = model_for_pred.predict_proba(Xva)[:, 1]
    best_thr, f1_val = find_best_threshold_f1(yva, p_val)
    print(f"\nBest threshold from DOLOS validation (max F1): {best_thr:.4f}")
    print(f"Validation F1 at best threshold: {f1_val:.4f}")

    # Evaluate on DOLOS TEST with tuned threshold
    p_te = model_for_pred.predict_proba(Xte)[:, 1]
    ...
    # feature importance (gain) from base model (calibrator doesn't have booster)
    ...
    print("Kaggle labels:", sorted(kag["label"].unique().tolist()))

    # align + impute using TRAIN means
    ...
    pk = model_for_pred.predict_proba(Xk)[:, 1]
